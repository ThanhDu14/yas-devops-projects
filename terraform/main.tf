...
# ==========================================
# 2. Create custom VPC and 2 Subnets
# ==========================================
...
# ==========================================
# 3. Create Cloud Router & Cloud NAT
# ==========================================
...
resource "google_compute_region_instance_group_manager" "yas_mig"{
  ...
  target_size = 1 #Default run 1 node 
  ...
  auto_healing_policies{
    ...
    initial_delay_sec = 600 #Wait 600sec for startup.sh to finish.
  }
...
# 7.5. Mở Firewall cho Google Health Check (Dải IP bắt buộc của Google: 35.191.0.0/16 và 130.211.0.0/22)
...
resource "google_storage_bucket" "frontend_bucket"{
  name = "yas-front-end-${var.project_id}" 
  location = var.region
  force_destroy = true
...
# ==========================================
# 9. GLOBAL LOAD BALANCER & CLOUD CDN
# ==========================================

# Backend-Bucket
resource "google_compute_backend_bucket" "frontend_backend_bucket" {
  name        = "yas-frontend-backend-bucket"
  bucket_name = google_storage_bucket.frontend_bucket.name
  enable_cdn  = true # Kích hoạt Cloud CDN tăng tốc độ tải trang
}
...
# 9.3. URL Map (Bộ định tuyến: /* -> Frontend GCS, /api/* & các Microservices -> Backend MIG)
resource "google_compute_url_map" "yas_url_map" {
  ...
  path_matcher {
    name            = "allpaths"
    default_service = google_compute_backend_bucket.frontend_backend_bucket.id
    # Định tuyến toàn bộ 15 Microservices + Keycloak xuống máy ảo Backend
    path_rule {
      paths = [
        "/api/*",
        "/product/*",
        ...
        "/recommendation/*",
        "/swagger-ui/*",
        "/realms/*",     # Keycloak Login / OAuth2
        "/resources/*",  # Keycloak Theme CSS
...
# 9.5. Global Forwarding Rule (Cấp 1 IP tĩnh toàn cầu duy nhất cho toàn bộ hệ thống)
